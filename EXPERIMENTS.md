...
### `postv12_progress_type_coverage_rerank_v1`

- Git tag: `postv12_progress_type_coverage_rerank_v1`.
- Adds `value_model_frontfill_progress_diverse`, a rerank mode that keeps
  v12 value-model frontfill, then inserts coverage candidates from construction
  families that online DDAR-progress signals show are under-ranked, before the
  secondary value-model coverage pass.
- Scout and stage4 defaults switch from `front12` to `front8 + progress
  coverage`, preserving strong v12 candidates while leaving DDAR budget for
  high-progress families such as `on_tline+on_tline`, `on_bline+on_dia`,
  `on_bline`, `on_bline+on_bline`, and `on_line+on_line`.
- Motivation: in the active v12 clean trace for `translated_imo_2009_p2`, the
  largest progress-delta candidates at depth 2 had low rerank scores, while
  lower-progress candidates such as `on_line+on_tline` were scored much higher.
  This strategy gives those mined high-progress types explicit evaluation
  coverage without retraining or interrupting the active benchmark.

### `postv12_progress_prefix_sampling_v1`

- Git tag: `postv12_progress_prefix_sampling_v1`.
- Adds `mixed_progress_constructive` candidate prompt sampling. It keeps the
  existing balanced constrained and mixed constructive prefixes, then adds
  direct constructive prefixes for `on_bline`, `on_dia`, and `eqdistance`.
- Parameterizes `CLEAN_CANDIDATE_PROMPT_SAMPLING` in the postrun clean rerun
  script; scout and stage4 clean defaults now use `mixed_progress_constructive`.
- Motivation: the active `translated_imo_2009_p2` trace still spends many
  depth-3 generations on `eqangle3`, while high-progress mined families include
  `on_bline`, `on_bline+on_dia`, `on_bline+on_bline`, and
  `eqdistance+on_circle`. The new sampling prefixes give those families a
  better chance to appear before reranking.

### `postv12_progress_beam_score_v1`

- Git tag: `postv12_progress_beam_score_v1`.
- Adds `rerank_plus_progress` as a candidate beam score. It keeps the reranker
  score as the base ordering signal, then adds a capped log-scaled bonus from
  DDAR progress: `added_dependencies(candidate) - added_dependencies(root)`.
- Adds `candidate_beam_add` events with the final beam score, progress delta,
  and progress bonus, so later traces can show whether high-progress states are
  actually surviving into the next search depth.
- Fixes `candidate_decode_beam_limit` pruning logs to unpack the current
  4-field beam state. This matters because the post-v12 scout/stage4 queues run
  with decode beam limiting enabled.
- Scout and post-v12 stage4 defaults now use
  `CANDIDATE_BEAM_SCORE=rerank_plus_progress`,
  progress weight `0.6`, and cap `4.0`. The base postrun script exposes the
  same parameters but keeps default weight `0.0` for backwards compatibility.
- Motivation: completed traces for `translated_imo_2008_p1a`,
  `translated_imo_2008_p1b`, and `translated_imo_2009_p2` contain many
...
### `postv12_depth_type_cap_v1`

- Git tag: `postv12_depth_type_cap_v1`.
- Adds `--candidate_depth_type_eval_cap`. When a depth-level candidate pool is
  larger than the eval budget, candidates from a construction type above the cap
  are delayed so more construction families receive DDAR evaluation slots.
- The gate is soft: delayed candidates are used to fill remaining slots if the
  under-cap candidates do not fill the eval budget. Pruned over-cap candidates
  are logged as `reason=depth_type_cap_pruned`.
- Defaults remain disabled in the base postrun script. The waiting scout uses
  depth eval 16 with type cap 4; the post-v12 stage4 clean rerun uses depth eval
  24 with type cap 6.
...
### `postv12_fact_context_top12_v1`

- Git tag: `postv12_fact_context_top12_v1`.
- Parameterizes `--lm_fact_context_top_k` in scout and clean rerun scripts.
  The base postrun clean script defaults to 8 for backwards compatibility.
- Waiting scout and post-v12 stage4 defaults use 12 DDAR facts in LM prompts.
- Motivation: completed traces for `translated_imo_2008_p1a`,
  `translated_imo_2008_p1b`, and `translated_imo_2009_p2` show many
