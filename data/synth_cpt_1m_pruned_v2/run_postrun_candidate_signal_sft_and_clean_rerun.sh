...
SIGNAL_MIN_PROGRESS_GOAL_FACT_OVERLAP=${SIGNAL_MIN_PROGRESS_GOAL_FACT_OVERLAP:-0}
SIGNAL_PROGRESS_GOAL_FACT_OVERLAP_WEIGHT=${SIGNAL_PROGRESS_GOAL_FACT_OVERLAP_WEIGHT:-0}
SIGNAL_MIN_PROGRESS_GOAL_FACT_SCORE=${SIGNAL_MIN_PROGRESS_GOAL_FACT_SCORE:-0}
SIGNAL_PROGRESS_GOAL_FACT_SCORE_WEIGHT=${SIGNAL_PROGRESS_GOAL_FACT_SCORE_WEIGHT:-0}
SIGNAL_STORE_CANDIDATE_FACT_CONTEXT=${SIGNAL_STORE_CANDIDATE_FACT_CONTEXT:-0}
...
CLEAN_CANDIDATE_ADAPTIVE_TYPE_PENALTY_DDAR_ERRORS=${CLEAN_CANDIDATE_ADAPTIVE_TYPE_PENALTY_DDAR_ERRORS:-0}
CLEAN_CANDIDATE_ADAPTIVE_TYPE_PENALTY_DDAR_ERROR_REASONS=${CLEAN_CANDIDATE_ADAPTIVE_TYPE_PENALTY_DDAR_ERROR_REASONS:-timeout}
...
CLEAN_CANDIDATE_SEED_AUX_FILE=${CLEAN_CANDIDATE_SEED_AUX_FILE:-}
CLEAN_CANDIDATE_SEED_AUX_LIMIT=${CLEAN_CANDIDATE_SEED_AUX_LIMIT:-8}
CLEAN_CANDIDATE_SEED_AUX_FRONTFILL_LIMIT=${CLEAN_CANDIDATE_SEED_AUX_FRONTFILL_LIMIT:-8}
...
export SIGNAL_MIN_PROGRESS_GOAL_FACT_OVERLAP SIGNAL_PROGRESS_GOAL_FACT_OVERLAP_WEIGHT
export SIGNAL_MIN_PROGRESS_GOAL_FACT_SCORE SIGNAL_PROGRESS_GOAL_FACT_SCORE_WEIGHT
export SIGNAL_STORE_CANDIDATE_FACT_CONTEXT
...
SIGNAL_FACT_CONTEXT_ARGS=()
if [ "$SIGNAL_STORE_CANDIDATE_FACT_CONTEXT" = "1" ]; then
  SIGNAL_FACT_CONTEXT_ARGS=(--store_candidate_fact_context)
fi
...
python -u "$SCRIPT_DIR/build_aux_sft_from_candidate_signals.py" \
  ...
  --min_progress_goal_fact_overlap "$SIGNAL_MIN_PROGRESS_GOAL_FACT_OVERLAP" \
  --progress_goal_fact_overlap_weight "$SIGNAL_PROGRESS_GOAL_FACT_OVERLAP_WEIGHT" \
  --min_progress_goal_fact_score "$SIGNAL_MIN_PROGRESS_GOAL_FACT_SCORE" \
  --progress_goal_fact_score_weight "$SIGNAL_PROGRESS_GOAL_FACT_SCORE_WEIGHT" \
...
if [ "$RUN_CLEAN_RERUN" = "1" ]; then
  ...
  if [ "$CLEAN_CANDIDATE_ADAPTIVE_TYPE_PENALTY" = "1" ]; then
    ...
    if [ "$CLEAN_CANDIDATE_ADAPTIVE_TYPE_PENALTY_DDAR_ERRORS" = "1" ]; then
      CLEAN_ADAPTIVE_TYPE_PENALTY_ARGS+=(
        --candidate_adaptive_type_penalty_ddar_errors
        --candidate_adaptive_type_penalty_ddar_error_reasons "$CLEAN_CANDIDATE_ADAPTIVE_TYPE_PENALTY_DDAR_ERROR_REASONS"
      )
    fi
  fi
