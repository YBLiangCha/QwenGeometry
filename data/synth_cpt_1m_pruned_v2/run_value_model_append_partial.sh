...
VALUE_PROGRESS_MIN_ADDED_DEPENDENCIES=${VALUE_PROGRESS_MIN_ADDED_DEPENDENCIES:-10}
VALUE_PROGRESS_MIN_ROOT_DELTA=${VALUE_PROGRESS_MIN_ROOT_DELTA:-40}
VALUE_PROGRESS_MAX_ELAPSED_SEC=${VALUE_PROGRESS_MAX_ELAPSED_SEC:-120}
VALUE_PROGRESS_MIN_EFFICIENCY=${VALUE_PROGRESS_MIN_EFFICIENCY:-0.5}
VALUE_MAX_PROGRESS_POSITIVES_PER_PROBLEM=${VALUE_MAX_PROGRESS_POSITIVES_PER_PROBLEM:-16}
VALUE_MAX_PROGRESS_POSITIVES_PER_TYPE=${VALUE_MAX_PROGRESS_POSITIVES_PER_TYPE:-32}
VALUE_SOLVED_POSITIVE_REPEAT=${VALUE_SOLVED_POSITIVE_REPEAT:-16}
VALUE_DISABLE_PROGRESS_POSITIVES=${VALUE_DISABLE_PROGRESS_POSITIVES:-0}
...
BUILD_ARGS=(
  ...
  --progress_positive_min_added_dependencies "$VALUE_PROGRESS_MIN_ADDED_DEPENDENCIES"
  --progress_positive_min_root_delta "$VALUE_PROGRESS_MIN_ROOT_DELTA"
  --progress_positive_max_elapsed_sec "$VALUE_PROGRESS_MAX_ELAPSED_SEC"
  --progress_positive_min_efficiency "$VALUE_PROGRESS_MIN_EFFICIENCY"
  --max_progress_positives_per_problem "$VALUE_MAX_PROGRESS_POSITIVES_PER_PROBLEM"
  --max_progress_positives_per_type "$VALUE_MAX_PROGRESS_POSITIVES_PER_TYPE"
  --solved_positive_repeat "$VALUE_SOLVED_POSITIVE_REPEAT"
)
if [ "$INCLUDE_UNEVALUATED_VALID" = "1" ]; then
  BUILD_ARGS+=(--include_unevaluated_valid)
fi
if [ "$VALUE_DISABLE_PROGRESS_POSITIVES" = "1" ]; then
  BUILD_ARGS+=(--disable_progress_positives)
fi
